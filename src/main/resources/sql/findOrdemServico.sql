DO @numero := IFNULL((SELECT max(numero) FROM ordensservico), 0);

INSERT INTO ordensservico (user_id, transferencia_id, numero,
                           data_hora, data_hora_conf, version, created_at, updated_at)
  SELECT
    IFNULL(O.user_id, null)                     AS user_id,
    IFNULL(O.transferencia_id, T.id)            AS transferencia_id,
    ...
    IFNULL(O.version, 0)                        AS version,
    IFNULL(O.created_at, current_timestamp)     AS created_at,
